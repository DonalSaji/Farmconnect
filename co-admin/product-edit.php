    ...
    if($valid == 1) {
    	...
    	if( isset($_FILES['photo']["name"]) && isset($_FILES['photo']["tmp_name"]) )
        {
            ...
            if(isset($final_name1)) {
		        ...
		        {
		        	$statement = $pdo->prepare("INSERT INTO tbl_product_photo (photo,product_id) VALUES (?,?)");
        ...
        if(isset($_POST['size'])) {
        	...
        	$statement = $pdo->prepare("DELETE FROM tbl_product_size WHERE product_id=?");
        	$statement->execute(array($_REQUEST['id']));

			foreach($_POST['size'] as $value) {
				$statement = $pdo->prepare("INSERT INTO tbl_product_size (size_id,product_id) VALUES (?,?)");
		...
		if(isset($_POST['color'])) {
			...
			$statement = $pdo->prepare("DELETE FROM tbl_product_color WHERE product_id=?");
        	$statement->execute(array($_REQUEST['id']));

			foreach($_POST['color'] as $value) {
				$statement = $pdo->prepare("INSERT INTO tbl_product_color (color_id,product_id) VALUES (?,?)");
				$statement->execute(array($value,$_REQUEST['id']));
			}
		} else {
			$statement = $pdo->prepare("DELETE FROM tbl_product_color WHERE product_id=?");
...
$statement = $pdo->prepare("SELECT * FROM tbl_product_size WHERE product_id=?");
$statement->execute(array($_REQUEST['id']));
$result = $statement->fetchAll(PDO::FETCH_ASSOC);							
foreach ($result as $row) {
	$size_id[] = $row['size_id'];
...
$statement = $pdo->prepare("SELECT * FROM tbl_product_color WHERE product_id=?");
$statement->execute(array($_REQUEST['id']));
$result = $statement->fetchAll(PDO::FETCH_ASSOC);							
foreach ($result as $row) {
						...
						<div class="form-group">
							...
							<div class="col-sm-4" style="padding-top:4px;">
								<table id="ProductTable" style="width:100%;">
			                        <tbody>
			                        	<?php
			                        	$statement = $pdo->prepare("SELECT * FROM tbl_product_photo WHERE product_id=?");
			                        	$statement->execute(array($_REQUEST['id']));
			                        	$result = $statement->fetchAll(PDO::FETCH_ASSOC);
			                        	foreach ($result as $row) {
			                        		?>
