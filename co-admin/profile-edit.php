	...
	if($_SESSION['user']['role'] == 'Super Admin') {
	    ...
	    if(empty($_POST['email'])) {
	    	...
	    	if (filter_var($_POST['email'], FILTER_VALIDATE_EMAIL) === false) {
		    	...
		    	$statement = $pdo->prepare("SELECT * FROM admin WHERE email=? and email!=?");
	    ...
	    if($valid == 1) {
			...
			$statement = $pdo->prepare("UPDATE admin SET full_name=?, email=?, phone=? WHERE id=?");
		...
		// updating the database
		$statement = $pdo->prepare("UPDATE admin SET phone=? WHERE id=?");
...
if(isset($_POST['form2'])) {
    ...
    if($valid == 1) {
        ...
        // updating the database
		$statement = $pdo->prepare("UPDATE admin SET photo=? WHERE id=?");
